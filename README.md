...
Иногда мне необходимо работать с удаленными серверами, не имеющими графического интерфейса. В этом случае полезно иметь возможность использовать команды bash. Они позволяют выполнять обычные действия, такие как создание, редактирование, удаление файлов и папок через CLI.

Вот некоторые команды, которые могут помочь. 
...
##### WРабота с файлами и каталогами
```bash
~                                     # Home directory 
